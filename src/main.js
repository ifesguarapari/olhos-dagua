...
const GOAL_SPRITE_SRC = `${import.meta.env.BASE_URL}assets/joana-idle.gif`;
const PLAYER_IDLE_SPRITE_SRC = `${import.meta.env.BASE_URL}assets/conceicao-idle.gif`;
const PLAYER_WALK_SPRITE_SRC = `${import.meta.env.BASE_URL}assets/conceicao-walk.gif`;
const PLAYER_CONGRATULATE_SPRITE_SRC = `${import.meta.env.BASE_URL}assets/conceicao-congratulate.gif`;
const PLAYER_MOVE_DURATION = 1800;
const PLAYER_BLOCKED_MOVE_DURATION = 900;
const PLAYER_TURN_DURATION = 220;
const PLAYER_SPRITE_MAX_HEIGHT_RATIO = 1.88;
const PLAYER_SPRITE_FOOT_OFFSET_RATIO = 0.42;
const GOAL_SPRITE_SIZE_MULTIPLIER = 1.25;
const GOAL_SPRITE_MAX_HEIGHT_RATIO = PLAYER_SPRITE_MAX_HEIGHT_RATIO * GOAL_SPRITE_SIZE_MULTIPLIER;
const GOAL_SPRITE_FOOT_OFFSET_RATIO = 0.42;
const A14_LEVEL_INDEX = 13;
const A17_LEVEL_INDEX = 16;
const B2_LEVEL_INDEX = 21;
const B3_LEVEL_INDEX = 22;
const GOAL_SPRITE_MIRROR_OVERRIDES = new Map([
  [A14_LEVEL_INDEX, false],
  [A17_LEVEL_INDEX, true],
  [B2_LEVEL_INDEX, true],
  [B3_LEVEL_INDEX, false],
]);
...
const goalSpriteElement = document.getElementById("goalSprite");
const playerSpriteElement = document.getElementById("playerSprite");
...
let playerSpriteMode = "idle";
let playerVisualPosition = null;
let goalSpriteLoadFailed = false;
let playerSpriteLoadFailed = false;
...
function goalCanvasPosition() {
  const tile = tileSize();
  const goal = currentLevel().goal;

  return {
    x: goal.x * tile + tile / 2,
    y: goal.y * tile + tile / 2,
  };
}

function shouldMirrorGoalSprite() {
  if (GOAL_SPRITE_MIRROR_OVERRIDES.has(currentLevelIndex)) {
    return GOAL_SPRITE_MIRROR_OVERRIDES.get(currentLevelIndex);
  }

  const startDirection = currentLevel().start.dir;
  const oppositeDirection = (startDirection + 2) % 4;

  return oppositeDirection === 2;
}

function updateGoalSpriteElement(offsetX = 0, offsetY = 0) {
  if (!goalSpriteElement || goalSpriteLoadFailed) return;

  const tile = tileSize();
  const position = goalCanvasPosition();
  const footY = position.y + tile * GOAL_SPRITE_FOOT_OFFSET_RATIO;
  const spriteHeight = tile * GOAL_SPRITE_MAX_HEIGHT_RATIO;

  if (goalSpriteElement.dataset.src !== GOAL_SPRITE_SRC) {
    goalSpriteLoadFailed = false;
    goalSpriteElement.classList.remove("hidden");
    goalSpriteElement.dataset.src = GOAL_SPRITE_SRC;
    goalSpriteElement.src = GOAL_SPRITE_SRC;
  }

  goalSpriteElement.style.left = `${((position.x + offsetX) / CANVAS_SIZE) * 100}%`;
  goalSpriteElement.style.top = `${((footY + offsetY) / CANVAS_SIZE) * 100}%`;
  goalSpriteElement.style.height = `${(spriteHeight / CANVAS_SIZE) * 100}%`;
  goalSpriteElement.classList.toggle("is-flipped", shouldMirrorGoalSprite());
}

function playerSpriteSrc(mode) {
  if (mode === "walk") return PLAYER_WALK_SPRITE_SRC;
  if (mode === "congratulate") return PLAYER_CONGRATULATE_SPRITE_SRC;

  return PLAYER_IDLE_SPRITE_SRC;
}

function setPlayerSpriteMode(mode, { restart = false } = {}) {
  playerSpriteMode = mode;
  if (!playerSpriteElement) return;

  const src = playerSpriteSrc(mode);
  const shouldChangeSource = restart || playerSpriteElement.dataset.src !== src;

  playerSpriteElement.classList.toggle("is-idle", mode === "idle");
  playerSpriteElement.classList.toggle("is-walking", mode === "walk");
  playerSpriteElement.classList.toggle("is-congratulating", mode === "congratulate");

  if (shouldChangeSource) {
    playerSpriteLoadFailed = false;
    playerSpriteElement.classList.remove("hidden");
    playerSpriteElement.dataset.src = src;
    playerSpriteElement.src = src;
  }

  updatePlayerSpriteElement();
}

function shouldMirrorPlayerSprite() {
  return directionAtOffset(0).dx < 0;
}

function updatePlayerSpriteElement(offsetX = 0, offsetY = 0) {
  if (!playerSpriteElement || playerSpriteLoadFailed) return;

  const tile = tileSize();
  const position = playerCanvasPosition();
  const footY = position.y + tile * PLAYER_SPRITE_FOOT_OFFSET_RATIO;
  const spriteHeight = tile * PLAYER_SPRITE_MAX_HEIGHT_RATIO;

  playerSpriteElement.style.left = `${((position.x + offsetX) / CANVAS_SIZE) * 100}%`;
  playerSpriteElement.style.top = `${((footY + offsetY) / CANVAS_SIZE) * 100}%`;
  playerSpriteElement.style.height = `${(spriteHeight / CANVAS_SIZE) * 100}%`;
  playerSpriteElement.classList.toggle("is-flipped", shouldMirrorPlayerSprite());
}

function startPlayerSpriteLoop() {
  updatePlayerSpriteElement();
}

function stopPlayerSpriteLoop() {
  updatePlayerSpriteElement();
}
...
function resetPlayerToLevel() {
  ...
  playerVisualPosition = null;
  updateGoalSpriteElement();
  setPlayerSpriteMode("idle");
...
function animateMoveAction({ keepWalking = false } = {}) {
  const from = { x: player.x, y: player.y };
  const next = frontCell(0);
  const direction = directionAtOffset(0);
  const isMoveBlocked = isBlocked(next.x, next.y);
  const duration = isMoveBlocked ? PLAYER_BLOCKED_MOVE_DURATION : PLAYER_MOVE_DURATION;

  setPlayerSpriteMode("walk", { restart: !keepWalking });
  ...
  return new Promise((resolve) => {
    ...
    function animate(now) {
      ...
      const eased = easeInOutSine(progressValue);

      if (isMoveBlocked) {
        const bump = Math.sin(progressValue * Math.PI) * 0.18;
        playerVisualPosition = {
          x: from.x + direction.dx * bump,
          y: from.y + direction.dy * bump,
        };
      } else {
        playerVisualPosition = {
...
async function animateVisualAction(action, { keepWalking = false } = {}) {
  if (action.type === "move") {
    await animateMoveAction({ keepWalking });
    return;
  }
  ...
  draw();
  await wait(PLAYER_TURN_DURATION);
}
...
function draw(offsetX = 0, offsetY = 0) {
  ...
  ctx.restore();
  updateGoalSpriteElement(offsetX, offsetY);
  updatePlayerSpriteElement(offsetX, offsetY);
}
...
function drawEmptyStage() {
  ...
  goalSpriteElement?.classList.add("hidden");
  playerSpriteElement?.classList.add("hidden");
}
...
function drawGoal() {
  const tile = tileSize();
  const detail = tile / 64;
  const { x, y } = goalCanvasPosition();

  ctx.save();

  if (goalSpriteElement && !goalSpriteLoadFailed) {
    ctx.fillStyle = "rgba(0,0,0,0.16)";
    ctx.beginPath();
    ctx.ellipse(x + tile * 0.04, y + tile * 0.32, tile * 0.28, tile * 0.08, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
    return;
  }
...
function drawPlayerShadow(x, y, tile) {
  ctx.save();
  ctx.fillStyle = "rgba(0,0,0,0.2)";
  ctx.beginPath();
  ctx.ellipse(x + tile * 0.05, y + tile * 0.34, tile * 0.34, tile * 0.1, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.restore();
}

function drawPlayerDirectionMarker(x, y, tile) {
  ctx.save();
  ctx.translate(x, y);
  ctx.rotate(((player.angle ?? player.dir * 90) * Math.PI) / 180);

  ctx.fillStyle = "rgba(36, 93, 35, 0.72)";
  ctx.strokeStyle = "rgba(255, 255, 255, 0.86)";
  ctx.lineWidth = Math.max(1.25, tile * 0.025);
  ctx.beginPath();
  ctx.moveTo(tile * 0.36, 0);
  ctx.lineTo(tile * 0.16, -tile * 0.11);
  ctx.lineTo(tile * 0.16, tile * 0.11);
  ctx.closePath();
  ctx.fill();
  ctx.stroke();
...
function drawFallbackPlayer(x, y, tile) {
  const detail = tile / 64;
...
function drawPlayer() {
  const tile = tileSize();
  const { x, y } = playerCanvasPosition();

  if (!playerSpriteElement || playerSpriteLoadFailed) {
    drawFallbackPlayer(x, y, tile);
    return;
  }

  drawPlayerShadow(x, y, tile);
  drawPlayerDirectionMarker(x, y, tile);
}
...
function stopAllEffects({ restartPlayerLoop = true } = {}) {
  stopAnimation("success");
  stopAnimation("error");
  stopAnimation("warning");
  draw();
  if (restartPlayerLoop) {
    startPlayerSpriteLoop();
  }
}

function startSuccessEffect() {
  stopPlayerSpriteLoop();
  stopAllEffects({ restartPlayerLoop: false });
  const start = performance.now();
  const duration = 1300;
...
async function evaluateResult() {
  ...
  if (isGoal() && allStarsCollected() && hasFullStarScore()) {
    setPlayerSpriteMode("congratulate", { restart: true });
...
async function animateActions(recordedActions) {
  ...
  const keepWalking = recordedActions.some((action) => action.type === "move");
  if (keepWalking) {
    setPlayerSpriteMode("walk", { restart: true });
  }

  for (const action of recordedActions) {
    highlightLine(action.lineNumber);
    await animateVisualAction(action, { keepWalking });
  }
...
async function stepDebug() {
  ...
  highlightLine(action.lineNumber);
  stopPlayerSpriteLoop();
  await animateVisualAction(action);
...
async function startApp() {
  ...
  setPlayerSpriteMode("idle", { restart: true });
  draw();
  await init();
}
...
if (goalSpriteElement) {
  goalSpriteElement.addEventListener("load", () => {
    goalSpriteLoadFailed = false;
    goalSpriteElement.classList.remove("hidden");
    updateGoalSpriteElement();
  });
  goalSpriteElement.addEventListener("error", () => {
    goalSpriteLoadFailed = true;
    goalSpriteElement.classList.add("hidden");
    draw();
  });
}
if (playerSpriteElement) {
  playerSpriteElement.addEventListener("load", () => {
    playerSpriteLoadFailed = false;
    playerSpriteElement.classList.remove("hidden");
    updatePlayerSpriteElement();
  });
  playerSpriteElement.addEventListener("error", () => {
    playerSpriteLoadFailed = true;
    playerSpriteElement.classList.add("hidden");
    draw();
  });
}
